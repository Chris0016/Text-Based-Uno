...
public class UnoDeck {
	...
	public static ArrayList<String> playerDeck = new ArrayList<String>();// use the Arrays.asList to transfer cards from
																			// cardSuits
	...
	public static void cpuPlay(ArrayList<String> playerID) {
		 ...
		 * Aim: Iterate through deck. If any card from deck matches the number on the
		 * top card then it will be put down and turn is over.
		 * If no cards match the number then iterate through deck for at least one card
		 * matching color then break.
		 * If no cards with same color are found(considering that cards with the same
		 * number were looked for) then pick up code is run(while loop).
		 * 
		 * This was accomplished by using a variable "count" to check if any cards were
		 * placed after the checks. If count was zeroo then the
			...
			if (count == 0) {
				for (byte i = 1; i < playerID.size(); i++) {
					if ((playerID.get(i).substring(0, 1)).equals(lastCard(playedDeck).substring(0, 1))) {
						...
						/*
						 * For usual cards the number or the color has to match the card that is being
						 * played.
						 * Only one that matches the color can be played or multiple that match the
						 * color can be played.
						 * 
						 * For special cards (plus2/4, reverse):
						 * For special cards, if the top card is not a special and it matches the color
						 * of the special
						 * played then the a card with the same color that is special can be played and
						 * then multiple of the same type can be played after no matter the color.
						 */
	...
	public static void setTC() {
		tC = playedDeck.get(playedDeck.size() - 1);
	}

	public static void printTC() {
		System.out.println("Top card: " + tC + "\n");
	}
	...
	public void setPlayedDeck() {
		...
		/*
		 * Since the top card is equal to the last card of the played deck, this method
		 * initializes the playedDeck
